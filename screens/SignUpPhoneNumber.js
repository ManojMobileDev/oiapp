    ...
    <View style={styles.container}>
        ...
        <KeyboardAvoidingView behavior={"position"}>
        {
            ...
            <Animatable.View key={key} animation={sent?'fadeInRight':'fadeOutRight'}>
                ...
                <Animatable.Text animation={'fadeInRight'} style={styles.subtitle}>{i18n.t('phone.subtitle')} {<Text onPress={()=>{setCode(true);checkUser();setKey(key+1)}} style={{color:'#4987F7'}}>{i18n.t('phone.subtitle2')}</Text>}  </Animatable.Text>
            ...
            <Animatable.View key={key} style={{paddingTop:70}} animation={sent==false?'fadeInRight':'fadeOutRight'}>
                ...
                <Animatable.View key={key2} animation={error?'shake':'fadeIn'} style={[styles.inputView,{borderColor:error?'red':'black'}]}>
                    ...
                    <TextInput
                        style={styles.input}
                        onChangeText={(text)=>{setNumber(text);setExist(false);setError(false)}}
                        value={number}
                ...
                {
                    exist?
                    <Animatable.Text animation={'fadeIn'} style={[styles.subtitle,{fontSize:12,marginTop:10,height:30,marginBottom:-15}]}>{i18n.t('phone.subtitle3')}</Animatable.Text>
                    :
                    <Text style={[styles.subtitle,{fontSize:12,marginTop:10,height:30,marginBottom:-15}]}></Text>
                }
                
                <TouchableHighlight style={[styles.button,{marginTop:30}]} onPress={()=>{checkUser();}}>
                    <View style={styles.buttonView}>
